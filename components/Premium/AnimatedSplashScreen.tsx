...
const AnimatedSplashScreen: React.FC<AnimatedSplashScreenProps> = ({ onAnimationComplete, isReadyToStart }) => {
    ...
    useEffect(() => {
        if (isReadyToStart) {
            // Fade segments in quickly
            segmentOpacity.value = withTiming(1, { duration: 400, easing: Easing.out(Easing.ease) });

            // Stage 1: Assembly – scattered pieces converge to center over 2.4s
            assembleProgress.value = withDelay(200, withTiming(1, {
                duration: 2400,
    ...
    // Individual segment animated styles
    const segStyle0 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[0][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[0][1], 0]) } as const,
        ] as any
    }));
    const segStyle1 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[1][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[1][1], 0]) } as const,
        ] as any
    }));
    const segStyle2 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[2][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[2][1], 0]) } as const,
        ] as any
    }));
    const segStyle3 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[3][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[3][1], 0]) } as const,
        ] as any
    }));
    const segStyle4 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[4][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[4][1], 0]) } as const,
        ] as any
    }));
    const segStyle5 = useAnimatedStyle(() => ({
        opacity: segmentOpacity.value * (1 - showFinal.value),
        transform: [
            { translateX: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[5][0], 0]) } as const,
            { translateY: interpolate(assembleProgress.value, [0, 1], [SCATTER_POSITIONS[5][1], 0]) } as const,
        ] as any
    }));

    const segStyles = [segStyle0, segStyle1, segStyle2, segStyle3, segStyle4, segStyle5];

    // Final merged logo style: hidden until assembly is done
