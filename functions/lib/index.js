...
exports.sendPasswordResetEmailBranded = (0, https_1.onCall)(async (request) => {
    ...
    if (recentAttempts.length >= 3) {
        throw new https_1.HttpsError('resource-exhausted', 'יותר מדי ניסיונות לאיפוס סיסמה. נסה שוב בעוד שעה.');
    }
...
// ══════════════════════════════════════════════════════════════════════════════
exports.sendWelcomeEmail = (0, firestore_2.onDocumentCreated)('users/{userId}', async (event) => {
...
// ══════════════════════════════════════════════════════════════════════════════
exports.onFamilyInviteCreated = (0, firestore_2.onDocumentCreated)('invites/{inviteCode}', async (event) => {
    const snapshot = event.data;
    if (!snapshot)
        return;
 ...
 */
exports.joinFamilyByCode = (0, https_1.onCall)(async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError('unauthenticated', 'יש להתחבר למערכת');
    const { code, force = false } = request.data;
    ...
    if (inviteSnap.exists) {
        ...
        if (new Date() > expiresAt) {
            throw new https_1.HttpsError('deadline-exceeded', 'קוד ההזמנה פג תוקף');
        }
        if (invite.used) {
            throw new https_1.HttpsError('already-exists', 'קוד ההזמנה כבר נוצל');
        }
        if (invite.createdBy === uid) {
            throw new https_1.HttpsError('permission-denied', 'לא ניתן להצטרף להזמנה שיצרת בעצמך');
        }
        const { familyId, childId } = invite;
        const familySnap = await db.doc(`families/${familyId}`).get();
        if (!familySnap.exists)
            throw new https_1.HttpsError('not-found', 'המשפחה לא נמצאה');
    ...
    if (familyQuery.empty) {
        throw new https_1.HttpsError('not-found', 'קוד הזמנה לא תקין');
    }
 ...
 */
async function _leaveFamily(uid, familyId) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const isLastAdmin = members[uid]?.role === 'admin' &&
        ...
        tx.update(db.doc(`users/${uid}`), { familyId: firestore_1.FieldValue.delete() });
    });
}
/**
 ...
 */
exports.onChatMessageCreated = (0, firestore_2.onDocumentCreated)('chats/{chatId}/messages/{messageId}', async (event) => {
    const snapshot = event.data;
    if (!snapshot)
        return;
